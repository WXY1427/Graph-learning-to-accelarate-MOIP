...
class MOIPtimiser:
    ...
    def __init__(self, model):
        ...
        self._init_M()
        self.fixed_part =self._new_empty_objective_model()
        self.obj_exp =self._objective_expressions()
    ...
    ####add to extract feats of IP constraints
    def _extract_vars(self, source):
        coeffs_in_objs = []
        for N in range(self._num_obj):
            coeffs_in_obj = []
            source_objective = source.getObjective(N)
            for i in range(source_objective.size()):
                #var = source_objective.getVar(i)
                coeffs_in_obj.append(source_objective.getCoeff(i))
            coeffs_in_objs.append(coeffs_in_obj)
        return np.asarray(coeffs_in_objs)
    ...
    ####add to extract feats of IP constraints
    def _extract_constraints(self, source):
        matrix_and_RHS = []
        all_index = []
        for constr in source.getConstrs():
            row = []
            ...
            for i in range(constraint_expression.size()):
                var = constraint_expression.getVar(i)
                row_index.append(var.index)
                coeff = constraint_expression.getCoeff(i)
                row.append(coeff)
            row.append(constr.RHS)
            matrix_and_RHS.append(row)
            all_index.append(row_index)
        if len(matrix_and_RHS)>1:
            a_zeros = np.zeros((len(matrix_and_RHS), (len(matrix_and_RHS[0])-1)**2))
            np.put_along_axis(a_zeros, np.asarray(all_index), 1, axis=1)
            matrix_and_RHS = np.concatenate([a_zeros, np.ones((len(matrix_and_RHS),1))],axis=-1)
        return np.asarray(matrix_and_RHS)
    ...
    ####add to extract feats of IP
    def _extract_model(self):
        variable_feats = self._extract_vars(self._model)
        print(variable_feats.shape)
        constraint_feats = self._extract_constraints(self._model)
        print(constraint_feats.shape)
        objective_feats = 0
        return variable_feats, constraint_feats, objective_feats
    ...
    def _set_other_objectives_as_constraints_simple(self, model, k, u, strict_inequality=True):
        constraints = []
        for i in range(self._num_obj):
            if i != k:
                new_expression = self.obj_exp[i]
                rhs = u[i] - 0.5 if strict_inequality else u[i]
                constraint = model.addLConstr(new_expression, GRB.LESS_EQUAL, rhs)
                constraints.append(constraint)
        return constraints

    def _objective_expressions(self):
        expressions = []
        for i in range(self._num_obj):
            other_objective = self._model.getObjective(i)
            new_expression = self._new_expression_from_objective_convert(self._model, other_objective)
            expressions.append(new_expression)
        return expressions

    def _new_expression_from_objective_convert(self, model, objective):
        new_expression = gp.LinExpr()
        if self._model.ModelSense == GRB.MAXIMIZE:
