...
class MoipAlgorithmClass(click.ParamType):
    ...
    lookup = {
        ...
        Wu2022_gnn: Wu2022DirectMOIPtimiser_gnn
    }
...
@click.option('-a','--algorithm', default=MoipAlgorithmClass.TAMBY2020_DIRECT,
              help=f"Selected algorithm. Valid options are {MoipAlgorithmClass.valid_options}",
              type=MoipAlgorithmClass())

@click.option('--runtime', default=3600,
              help=f"Selected algorithm")


def main(filepath, algorithm, runtime):
    tick = time.time()
    moiptimiser = algorithm.from_lp_file(filepath)
    nds = moiptimiser.find_non_dominated_objective_vectors(runtime, filepath[-8:])
    tock = time.time()
    ...
    for nd in nds:
        #click.echo(repr(nd))
        click.echo(f"{repr(nd)},")

    click.echo(f"Non-dominated vectors found: {len(nds)}")
    click.echo(f"Runtime: {tock-tick}")
